...
class APIItemListTests(TestCase):
    ...
    def test_get_list_returns_correct_data_with_sort(self):       
        ...
        a2_data = {'created_by': 'cat',
                   'created_time': timezone.now(),
                   'abbreviation': 'TA2',
                   'full_name': 'A Test Author 2',
                   'language': 'lat'
                   }
        a2 = models.Author.objects.create(**a2_data)
        response = self.client.get('%s?_sort=full_name' % self.base_url.format('citations', 'author'))
        response_json = json.loads(response.content.decode('utf8'))
        self.assertEqual(response_json['count'], 2)
        self.assertEqual(len(response_json['results']), 2)
    ...
    def test_get_list_returns_correct_data_with_field_filtering(self):       
        ...
        a2 = models.Author.objects.create(**a2_data)
        response = self.client.get('%s?_fields=id,abbreviation' % self.base_url.format('citations', 'author'))
        response_json = json.loads(response.content.decode('utf8'))
        self.assertEqual(response_json['count'], 2)
        self.assertEqual(len(response_json['results']), 2)
    ...
    def test_get_list_returns_correct_data_with_pagination(self):       
        ...
        a2 = models.Author.objects.create(**a2_data)
        response = self.client.get('%s?limit=1' % self.base_url.format('citations', 'author'))
        response_json = json.loads(response.content.decode('utf8'))
        self.assertEqual(response_json['count'], 2)
...
class APIItemDetailTests(TestCase):
    ...
    def test_get_list_returns_correct_data(self):
        ...
        response = self.client.get(self.base_url.format('citations', 'author', a1.id))
        response_json = json.loads(response.content.decode('utf8'))
        self.assertEqual(response_json['id'], a1.id)
        self.assertEqual(response_json['full_name'], 'Test Author 1')
